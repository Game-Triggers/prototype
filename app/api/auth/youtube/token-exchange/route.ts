...
export async function POST(request: NextRequest) {
  try {
    ...
    const response = await fetch(`${NEST_API_URL}/auth/youtube/token-exchange`, {
      method: 'POST',
      headers: {
    ...
    if (!response.ok) {
      ...
      console.error(`[YouTube Token Exchange] Backend error (${response.status}):`, errorText);
    ...
    console.log('[YouTube Token Exchange] Success, user ID:', data.user?.id);
